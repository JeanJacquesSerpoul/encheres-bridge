...
func (e *Engine) respondMajor(p *playerState, M Suit) (Call, meaning) {
	...
	if tr.check(hl >= 11,
		...
		if ok {
			...
			e.gameForce[sideOf(p.seat)] = true
			tr.note("→ 2 sur 1, forcing de manche", "→ two-over-one, game forcing")
...
func (e *Engine) druryRebid(p *playerState, os Suit) (Call, meaning) {
	...
	tr := e.tr
	tr.note("le partenaire, main passée, a fait un Drury (fit et 11 HLD) : se situer",
		"partner, a passed hand, bid Drury (a fit and 11 HLD): place the hand")
	if c, mn, ok := e.drurySecondSuit(p, os, hld); tr.check(ok, "beau bicolore → nommer la deuxième couleur",
		"good two-suiter → bid the second suit", callOrEmpty(c, ok)) {
		return c, mn
	}
	...
	switch {
	case tr.check(hld >= 19, "19 HLD et plus → répétition à saut, ambition de chelem", "19+ HLD → jump repeat, slam ambition", pts(hld, "HLD")):
		c := bidSuit(3, os)
		if e.legal(p.seat, c) {
	...
	case tr.check(hld >= 17, "17-18 HLD → la manche", "17-18 HLD → game", pts(hld, "HLD")):
		c := bidSuit(4, os)
		if e.legal(p.seat, c) {
			return c, m(17, 18, "je veux jouer la manche, ne pensons pas au chelem",
				"bidding the game, no slam interest").withLen(os, 5)
		}
	case tr.check(hld >= 15, "15-16 HLD → 2♦, essai de manche", "15-16 HLD → 2♦, game try", pts(hld, "HLD")):
		c := bid(2, SDiamonds)
		if e.legal(p.seat, c) {
	...
	c := bidSuit(2, os)
	if e.legal(p.seat, c) {
		tr.note("ouverture minimale → arrêt au palier de 2", "minimum opening → stop at the two level")
...
func (e *Engine) druryShortRebid(p *playerState, os Suit) (Call, meaning) {
	tr := e.tr
	tr.note("le partenaire a répondu 2SA au Drury : 4 atouts et un singleton", "partner answered the Drury with 2NT: four trumps and a singleton")
	if tr.check(e.hldAgainstTheirBidding(p, os) <= 14, "ouverture minimale (14 HLD au plus) → arrêt au palier de 3",
		"minimum opening (14 HLD at most) → stop at the three level", pts(e.hldAgainstTheirBidding(p, os), "HLD")) {
		c := bidSuit(3, os)
		if e.legal(p.seat, c) {
	...
	tr.note("sinon → 3♣, quel est ton singleton ?", "otherwise → 3♣, which singleton?")
...
func (e *Engine) druryGameTryAnswer(p *playerState, os Suit) (Call, meaning) {
	...
	lvl := 2
	tr := e.tr
	tr.note("l'ouvreur demande la longueur du soutien (2♦ Drury)", "opener asks the length of the support (2♦ Drury)")
	switch {
	case tr.check(sup >= 5, "5 atouts → la manche", "five trumps → game", cards(p.hand, os)):
		lvl = 4
	case tr.check(sup == 4, "4 atouts → palier de 3", "four trumps → the three level", cards(p.hand, os)):
		lvl = 3
	default:
		tr.note("3 atouts → palier de 2", "three trumps → the two level")
	}
	c := bidSuit(lvl, os)
	if !e.legal(p.seat, c) {
		return e.concludeHandoff(p)
...
func (e *Engine) respondMinor(p *playerState, ms Suit) (Call, meaning) {
	...
	if tr.check(sup >= minSup,
		...
		tr.note("sinon, 6-10 HLD → soutien simple à 2", "otherwise, 6-10 HLD → single raise to two")
...
func (e *Engine) respondNT(p *playerState, base, oMin int) (Call, meaning) {
	...
	if tr.check((lh >= 5 && ls < 4) || (ls >= 5 && lh < 4),
		...
		tr.note("→ Texas pour la majeure cinquième", "→ transfer to the five-card major")
...
func (e *Engine) afterStrongMinorTexas(p *playerState, twoSuited bool) (Call, meaning) {
	...
	if e.tr.check(twoSuited, "bicolore 5-5 dans les mineures → 4♦, forcing de manche", "5-5 in the minors → 4♦, game forcing", shape(p.hand)) {
		c := bid(4, SDiamonds)
...
func (e *Engine) minorTransferAnswer(p *playerState, primary Suit) (Call, meaning) {
	h := p.hand
	tr := e.tr
	tr.note("le partenaire a fait un Texas mineur : rectifier", "partner made a minor transfer: complete it")
	if primary == Clubs {
		goodFit := h.Len(Clubs) >= 4 ||
			(h.Len(Clubs) == 2 && h.HasCard(Clubs, 'K')) ||
			(h.Len(Clubs) == 3 && h.HasCard(Clubs, 'Q'))
		if tr.check(goodFit, "bon fit à ♣ (4 cartes, Roi second ou Dame troisième) → refuser par 2SA",
			"a good club fit (four cards, king doubleton or queen third) → decline with 2NT", cards(h, Clubs)) {
			c := bid(2, SNoTrump)
			if e.legal(p.seat, c) {
	...
	cFR, cEN := callSym(c)
	tr.check(true, "rectifier au plus bas → "+cFR, "complete at the lowest level → "+cEN, cards(h, primary))
...
func (e *Engine) afterMinorTexasWeak(p *playerState) (Call, meaning) {
	tr := e.tr
	tr.note("Texas mineur avec une main faible : arrêt", "minor transfer with a weak hand: sign-off")
	if last, _, ok := e.lastBid(); tr.check(ok && last == bid(2, SNoTrump), "l'ouvreur a refusé par 2SA → revenir à 3♣",
		"opener declined with 2NT → back to 3♣", "") {
		c := bid(3, SClubs)
		if e.legal(p.seat, c) {
			return c, m(0, 7, "revient à 3T, arrêt absolu", "corrects back to 3C, an absolute sign-off")
		}
	}
	tr.note("→ Passe dans la mineure", "→ pass the minor")
...
func (e *Engine) afterMinorTexasGame(p *playerState, primary Suit, twoSuited bool) (Call, meaning) {
	h := p.hand
	tr := e.tr
	tr.note("Texas mineur avec des valeurs de manche : décrire la forme", "minor transfer with game values: describe the shape")
	if tr.check(twoSuited, "bicolore 5-5 dans les mineures → 3♦", "5-5 in the minors → 3♦", shape(h)) {
		c := bid(3, SDiamonds)
		if e.legal(p.seat, c) {
			e.gameForce[sideOf(p.seat)] = true
	...
	if !tr.check(ok, "un singleton ou une chicane à montrer", "a singleton or void to show", shape(h)) {
		tr.note("→ 3SA", "→ 3NT")
		c := bid(3, SNoTrump)
...
func (e *Engine) afterTransfer(p *playerState, M Suit, oMin int, completion Call) (Call, meaning) {
	...
	if last, lastSeat, ok := e.lastBid(); ok && lastSeat == partnerOf(p.seat) &&
		...
		if cMin, cMax := h.HLD(M)+partner.shownMin, h.HLD(M)+partner.shownMax; cMin >= 29 && (cMax >= 33 || cMin >= 31) {
			mark := e.tr.mark()
			e.tr.note("l'ouvreur a rectifié le Texas avec saut, et le chelem est en vue : les contrôles",
				"opener super-accepted the transfer, and slam is in view: control bids")
			if c, mn, ok := e.initiateControls(p, M, e.tr); ok {
				return c, mn
			}
			e.tr.rewind(mark)
			if c, mn, ok := e.concludeTry(p); ok {
				return c, mn
			}
		}
		e.tr.note("l'ouvreur a rectifié le Texas avec saut (4 atouts, maximum)", "opener super-accepted the transfer (four trumps, maximum)")
		if e.tr.check(hp >= 5, "5 H et plus → la manche", "5+ H → game", pts(hp, "H")) {
			return bidSuit(4, M), m(5, -1, "conclusion à la manche après la rectification à saut", "game after the super-accept").withLen(M, 5)
		}
		e.tr.note("moins de 5 H → Passe", "under 5 H → Pass")
		return passCall, m(0, 4, "arrêt, jeu trop faible malgré le fit", "sign-off, too weak even with the fit")
	}
	tr := e.tr
	sym := suitSymbol[M]
	tr.note("l'ouvreur a rectifié le Texas : décider selon les points et la longueur à "+sym,
		"opener completed the transfer: decide on points and "+sym+" length")
	if oMin >= 20 { // facing 2NT/2C strong: thresholds shrink
		switch {
		case tr.check(hp <= 3, "face à un 2SA fort : 0-3 H → Passe", "facing a strong 2NT: 0-3 H → Pass", pts(hp, "H")):
			return passCall, m(0, 3, "jeu trop faible pour la manche", "too weak for game")
		case tr.check(hp <= 10 && h.Len(M) >= 6, "4-10 H et 6 cartes → la manche 4"+sym, "4-10 H and six cards → game 4"+sym, pts(hp, "H")+", "+cards(h, M)):
			return bidSuit(4, M), m(4, 10, "conclusion à la manche, 6 atouts", "game with a six-card suit").withLen(M, 6)
		case tr.check(hp <= 10, "4-10 H et 5 cartes → 3SA, l'ouvreur choisit", "4-10 H and five cards → 3NT, opener chooses", pts(hp, "H")):
	...
	switch {
	case tr.check(hp >= 5 && hp <= 9 && h.Len(M) >= 6, "5-9 H et 6 cartes → proposer 3"+sym, "5-9 H and six cards → invite with 3"+sym, pts(hp, "H")+", "+cards(h, M)):
		return bidSuit(3, M), m(5, 9, "proposition de manche, 6 atouts", "game invitation, six trumps").withLen(M, 6).asInvite()
	case tr.check(hp <= 7, "0-7 H → Passe", "0-7 H → Pass", pts(hp, "H")):
		return passCall, m(0, 7, "arrêt, jeu faible", "sign-off, weak hand")
	case tr.check(hp <= 9, "8-9 H → 2SA, proposition", "8-9 H → 2NT, invitation", pts(hp, "H")):
		return bid(2, SNoTrump), m(8, 9, "2SA, 8-9H, proposition de manche", "2NT, 8-9, game invitation").asInvite()
	case tr.check(hp <= 15, "10-15 H → la manche (4"+sym+" avec 6 cartes, sinon 3SA)", "10-15 H → game (4"+sym+" with six cards, else 3NT)", pts(hp, "H")+", "+cards(h, M)):
		if h.Len(M) >= 6 {
	...
	default:
		tr.note("16 H et plus → 4SA quantitatif", "16+ H → quantitative 4NT")
...
func (e *Engine) afterStaymanDenial(p *playerState, base, askLevel int) (Call, meaning) {
	...
	lh, ls := h.Len(Hearts), h.Len(Spades)
	tr := e.tr
	tr.note("l'ouvreur a nié toute majeure quatrième sur le Stayman (2♦)", "opener denied a four-card major over Stayman (2♦)")

	if tr.check(hl >= 11, "11 HL et plus : la manche", "11+ HL: game", pts(hl, "HL")) {
		switch {
		case tr.check(lh == 4 && ls >= 5, "4 ♥ et 5 ♠ → chassé-croisé 3♥", "four ♥ and five ♠ → crisscross 3♥", cards(h, Hearts)+", "+cards(h, Spades)):
		...
		case tr.check(ls == 4 && lh >= 5, "4 ♠ et 5 ♥ → chassé-croisé 3♠", "four ♠ and five ♥ → crisscross 3♠", cards(h, Spades)+", "+cards(h, Hearts)):
	...
	if c, mn, ok := e.staymanNewMinor(p, askLevel); tr.check(ok, "mineure cinquième avec une courte (ou main de chelem) → la nommer, forcing de manche",
		"five-card minor with a short suit (or a slam hand) → bid it, game forcing", callOrEmpty(c, ok)) {
		return c, mn
	}
...
func (e *Engine) afterStaymanOneMajor(p *playerState, base, askLevel int, M Suit) (Call, meaning) {
	...
	tr := e.tr
	sym := suitSymbol[M]
	tr.note("l'ouvreur a montré 4 cartes à "+sym+" sur le Stayman", "opener showed four "+sym+" over Stayman")
	if tr.check(h.Len(M) >= 4, "4 cartes à "+sym+" : le fit est trouvé", "four "+sym+": the fit is found", cards(h, M)) {
		hld := h.HLD(M)
		...
		if tr.check(hld >= slamZone-3 && hld < slamZone, fmt.Sprintf("%d-%d HLD : juste sous le chelem", slamZone-3, slamZone-1),
			fmt.Sprintf("%d-%d HLD: just below slam", slamZone-3, slamZone-1), pts(hld, "HLD")) {
			if short, ok := splinterSuit(h, M); tr.check(ok, "un singleton ou une chicane → splinter", "a singleton or void → splinter", shape(h)) {
		...
		if tr.check(hld >= slamZone, fmt.Sprintf("%d HLD et plus : ambition de chelem", slamZone),
			fmt.Sprintf("%d+ HLD: slam ambition", slamZone), pts(hld, "HLD")) {
		...
		if e.legal(p.seat, c) {
			cFR, cEN := callSym(c)
			tr.note("sinon → "+cFR, "otherwise → "+cEN)
	...
	if c, mn, ok := e.staymanNewMinor(p, askLevel); tr.check(ok, "une mineure cinquième à montrer, forcing de manche",
		"a five-card minor to show, game forcing", callOrEmpty(c, ok)) {
		return c, mn
	}
	e.tr.note("après le Stayman, pas de fit majeur à jouer : réponse selon les points", "after Stayman, no major fit to play: response by points")
	return e.ntResponseLadder(p, base)
...
func (e *Engine) afterStaymanBothMajors(p *playerState, base, askLevel int) (Call, meaning) {
	...
	slamZone := 33 - e.ps[partnerOf(p.seat)].shownMin
	tr := e.tr
	tr.note("l'ouvreur a montré les deux majeures sur le Stayman : fit à "+suitSymbol[target],
		"opener showed both majors over Stayman: fit in "+suitSymbol[target])
	switch {
	case tr.check(hl >= slamZone, fmt.Sprintf("%d HL et plus → ambition de chelem", slamZone), fmt.Sprintf("%d+ HL → slam ambition", slamZone), pts(hl, "HL")):
	...
	case tr.check(hl >= slamZone-2, "juste sous le chelem → Texas, la manche", "just below slam → transfer, game", pts(hl, "HL")):
	...
	default:
		tr.note("sinon → transfert, l'ouvreur rectifie selon sa force", "otherwise → transfer, opener corrects by strength")
...
func (e *Engine) staymanBothMajorsAnswer(p *playerState, target Suit) (Call, meaning) {
	...
	tr := e.tr
	if tr.check(p.hand.H() >= p.shownMax, "maximum → rectifier avec saut", "maximum → jump completion", pts(p.hand.H(), "H")) {
	...
	if e.legal(p.seat, c) {
		tr.note("→ rectification simple", "→ plain completion")
...
func (e *Engine) afterStaymanMisereDoree(p *playerState, M Suit) (Call, meaning) {
	...
	tr := e.tr
	tr.note("misère dorée : l'ouvreur a répondu au Stayman", "misère dorée: opener answered Stayman")
	other := Hearts
	if M == Hearts {
		other = Spades
	}
	...
	if tr.check(last == bid(2, M.Strain()), "4 cartes dans ma majeure : fit neuvième → la manche", "four cards in my major: nine-card fit → game", "") {
		c := bidSuit(4, M)
		if e.legal(p.seat, c) {
	...
	if tr.check(last == bid(2, SNoTrump), "2SA : fit connu → Texas à 4", "2NT: known fit → transfer at the four level", "") {
	...
	if tr.check(last == bid(2, SDiamonds) || last == bid(2, other.Strain()), "pas de fit majeur montré → nommer ma cinquième (proposition)",
		"no major fit shown → bid my five-card major (invitation)", cards(p.hand, M)) {
		c := bid(2, M.Strain())
		if e.legal(p.seat, c) {
...
func (e *Engine) respondAfter2C(p *playerState) (Call, meaning) {
	...
	if sc.Call.IsBid() && sc.Call.Strain <= SSpades {
		...
		sup := h.Len(os)
		tr := e.tr
		scFR, scEN := callSym(sc.Call)
		tr.note("2♣ fort puis "+scFR+" du partenaire : soutenir, nommer sa couleur ou attendre",
			"strong 2♣ then "+scEN+" from partner: raise, name a suit or wait")
		if tr.check(sup >= 3, "3 cartes dans sa couleur", "three cards in partner's suit", cards(h, os)) {
			if tr.check(hl <= 7, "0-7 HL → soutien limité", "0-7 HL → limited raise", pts(hl, "HL")) {
				if os.IsMajor() {
			...
			tr.note("8 HL et plus → soutien, espoir de chelem", "8+ HL → raise, slam interest")
			return e.cheapestCall(os.Strain()), m(8, 40, "soutien, espoir de chelem", "raise, slam interest").withLen(os, 3).asForcing()
		}
		if s, n := h.Longest(), h.Len(h.Longest()); tr.check(n >= 5 && hl >= 5 && s != os, "couleur cinquième et 5 HL → la nommer, forcing",
			"five-card suit and 5 HL → bid it, forcing", shape(h)+", "+pts(hl, "HL")) {
		...
		if tr.check(waiting.Level == 2 && hl <= 7, "0-7 HL, 2SA encore disponible → enchère d'attente",
			"0-7 HL, 2NT still available → waiting bid", pts(hl, "HL")) {
			return waiting, m(0, 7, "enchère d'attente", "waiting bid").asForcing()
		}
		if tr.check(e.ntSafe(p), "Sans-Atout jouable → 3SA", "notrump playable → 3NT", "") {
			if waiting.Level == 2 {
		...
		tr.note("sinon → soutien faute de mieux", "otherwise → raise for want of anything better")
...
func (e *Engine) respondAces(p *playerState) (Call, meaning) {
	...
	tr := e.tr
	tr.note("le partenaire a ouvert de 2♦ fort : répondre par paliers d'As", "partner opened a strong 2♦: answer by ace steps")
	var held []string
	for _, a := range aces {
		held = append(held, "A"+suitSymbol[a])
	}
	acesVal := fmt.Sprint(len(aces))
	if len(held) > 0 {
		acesVal = strings.Join(held, " ")
	}
	mk := func(c Call, min, max int, fr, en string) (Call, meaning) {
		cFR, cEN := callSym(c)
		tr.check(true, fmt.Sprintf("%d As : %s → %s", len(aces), fr, cFR), fmt.Sprintf("%d aces: %s → %s", len(aces), en, cEN), acesVal)
...
func (e *Engine) respondMinorAffranchie(p *playerState) (Call, meaning) {
	...
	tr := e.tr
	tr.note("le partenaire a ouvert de 3SA (mineure septième affranchie)", "partner opened 3NT (a solid seven-card minor)")
	if !tr.check(h.H() >= 14 && short, "14 H et une courte → essai au-delà de 3SA",
		"14 H and a short suit → try beyond 3NT", pts(h.H(), "H")+", "+shape(h)) {
		tr.note("→ Passe, 3SA est le meilleur contrat", "→ pass, 3NT is the best contract")
	...
	tr.note("ancre à Trèfle (5♣ dès 17 H), le partenaire rectifie à Carreau", "clubs as the anchor (5♣ from 17 H), partner corrects to diamonds")
	mn := m(-1, -1,
...
func (e *Engine) affranchieRebid(p *playerState) (Call, meaning) {
	...
	tr := e.tr
	tr.note("3SA : mineure septième affranchie, le partenaire a répondu dans une mineure",
		"3NT: a solid seven-card minor, partner answered in a minor")
	if tr.check(Suit(last.Strain) == trueSuit, "c'est ma mineure → Passe", "it is my minor → Pass", cards(p.hand, trueSuit)) {
	...
	tr.note("→ rectifier vers ma mineure", "→ correct to my minor")
...
func (e *Engine) respondWeak2(p *playerState, M Suit) (Call, meaning) {
	...
	tr := e.tr
	sym := suitSymbol[M]
	tr.note("le partenaire a ouvert d'un 2 faible ("+"2"+sym+", 6 cartes, 6-10 H)", "partner opened a weak two (2"+sym+", six cards, 6-10 H)")
	switch {
	case tr.check(sup >= 4, "4 atouts → manche 4"+sym+", attaque ou défense", "four trumps → game 4"+sym+", to make or to sacrifice", cards(h, M)):
	...
	case tr.check(sup >= 2 && hld >= 18 && hld <= 20, "2 atouts et 18-20 HLD → manche directe", "two trumps and 18-20 HLD → direct game", cards(h, M)+", "+pts(hld, "HLD")):
		return bidSuit(4, M), m(18, 20, "manche directe en attaque, 18-20HLD", "direct raise to game, 18-20 HLD").withLen(M, sup)
	case tr.check(sup >= 2 && (hld >= 21 || (hld >= 15 && hld <= 17)), "2 atouts, 15-17 ou 21 HLD et plus → 2SA relais", "two trumps, 15-17 or 21+ HLD → 2NT relay", cards(h, M)+", "+pts(hld, "HLD")):
	...
	case tr.check(hl >= 13 && func() bool {
		...
		return false
	}(), "13 HL et une belle cinquième dans l'autre majeure → la nommer, forcing", "13 HL and a good five cards in the other major → bid it, forcing", pts(hl, "HL")):
		var s Suit
		if M == Hearts {
			s = Spades
		} else {
	...
	if tr.check(sup == 3, "3 atouts → prolonger le barrage (ou rencontre avec une belle mineure)",
		"three trumps → extend the preempt (or a meeting bid with a good minor)", cards(h, M)) {
		for _, s := range []Suit{Clubs, Diamonds} {
			if c, mn, ok := e.rencontreCall(p, M, s, 3, 1); ok {
				tr.check(true, "belle cinquième à "+suitSymbol[s]+" → rencontre", "good five cards in "+suitSymbol[s]+" → meeting bid", cards(h, s))
				return c, mn
			}
		}
		tr.note("→ 3"+sym, "→ 3"+sym)
	...
	if tr.check(hl >= 16 && h.IsRegular(), "16 HL et plus, régulière → 3SA", "16+ HL, balanced → 3NT", pts(hl, "HL")+", "+shape(h)) {
		return bid(3, SNoTrump), m(16, 40, "conclusion à 3SA", "3NT to play")
	}
	tr.note("rien à ajouter → Passe", "nothing to add → Pass")
...
func (e *Engine) respondStrongTwoInterference(p *playerState) (Call, meaning) {
	h := p.hand
	tr := e.tr
	tr.note("intervention sur l'ouverture forte du partenaire : réponse naturelle", "an overcall of partner's strong opening: natural response")
	if tr.check(h.H() < 5, "moins de 5 H → Passe", "under 5 H → Pass", pts(h.H(), "H")) {
	...
	if tr.check(bestLen > 0, "une belle cinquième → la nommer, forcing", "a good five-card suit → bid it, forcing", shape(h)) {
	...
	if tr.check(e.legal(p.seat, doubleCall), "sinon, 5 H → contre", "otherwise, 5 H → double", "") {
...
func (e *Engine) respondCompetitive(p *playerState) (Call, meaning) {
	...
	ocFR, ocEN := callSym(oc)
	tr.note("les adversaires sont intervenus sur l'ouverture ("+ocFR+") : réponse en compétition",
		"the opponents came in over the opening ("+ocEN+"): competitive response")
	...
	if oc.Strain <= SSpades {
		...
		if !(os.IsMajor() && tr.check(h.Len(os) >= 3, "3 cartes dans la majeure d'ouverture : le fit est déjà connu",
			"three cards in the opened major: the fit is already known", cards(h, os))) {
			best, bestLen := Suit(-1), 0
			for _, s := range []Suit{Spades, Hearts} {
			...
			if tr.check(best >= 0,
				"majeure libre : 5 cartes (4 sur un contre au palier de 1), 6 HL au palier de 1, 11 au palier de 2 → la nommer, forcing",
				"free major: five cards (four over a double at the one level), 6 HL at the one level, 11 at the two level → bid it, forcing",
				cards(h, Spades)+", "+cards(h, Hearts)+", "+pts(hl, "HL")) {
				c := e.cheapestCall(best.Strain())
	...
	if oc.Strain <= SSpades {
		...
		if c, mn, ok := e.competitiveRencontre(p, Suit(oc.Strain), doubled, rho); tr.check(ok,
			"fit et belle couleur annexe → enchère de rencontre en compétition",
			"fit and a good side suit → competitive fit-showing bid", callOrEmpty(c, ok)) {
			return c, mn
		}
	}
	...
	if oc.Strain <= SSpades {
		...
		if tr.check(sup >= minSup, fmt.Sprintf("fit : %d cartes et plus dans la couleur d'ouverture", minSup),
			fmt.Sprintf("fit: %d+ cards in the opened suit", minSup), cards(h, os)) {
			hld := h.HLD(os)
			// From 13 HLD game values are known: conclude directly instead of
			// a limit raise that a minimum opener would pass.
			if tr.check(hld >= 13, "13 HLD et plus : la manche est connue → décision sur la force combinée",
				"13+ HLD: game values are known → decision on combined strength", pts(hld, "HLD")) {
				mark := tr.mark()
				tr.in()
				if c, mn := e.concludeFrom(p, tr); c.Kind != KindPass {
					return c, mn
				}
				tr.rewind(mark)
			}
			...
			tr.check(byLaw, "loi des levées totales : 9 atouts valent le palier de 3, 10 le palier de 4",
				"law of total tricks: 9 trumps are worth the three level, 10 the four level",
				fmt.Sprintf("%d + %d %s", sup, partner.shownLens[os], suitSymbol[os]))
			switch {
			case tr.check(hld >= 11, "11 HLD et plus → soutien en compétition", "11+ HLD → competitive raise", pts(hld, "HLD")):
				c := raiseTo()
				if c.Level <= 4 {
			...
			case tr.check(hld >= 6, "6-10 HLD → soutien simple", "6-10 HLD → simple raise", pts(hld, "HLD")):
				c := raiseTo()
				if c.Level <= 4 {
	...
	if hl >= 6 {
		for _, s := range []Suit{Spades, Hearts, Diamonds, Clubs} {
			...
			if h.Len(s) >= 4 && c.Level == 1 {
				tr.check(true, "couleur quatrième au palier de 1, 6 HL → la nommer, forcing",
					"four-card suit at the one level, 6 HL → bid it, forcing", cards(h, s)+", "+pts(hl, "HL"))
	...
	if hl >= 11 {
		...
		if tr.check(bestLen > 0, "couleur cinquième au palier de 2, 11 HL → la plus longue, forcing",
			"five-card suit at the two level, 11 HL → the longest, forcing", pts(hl, "HL")) {
	...
	if tr.check(hl >= 8 && hl <= 10 && last.IsBid() && last.Strain <= SSpades && h.Stopper(Suit(last.Strain)),
		"8-10 HL et un arrêt dans leur couleur → 1SA", "8-10 HL and a stopper in their suit → 1NT", pts(hl, "HL")) {
		c := e.cheapestCall(SNoTrump)
		if c.Level == 1 {
			return c, m(8, 10, "1SA, 8-10HL avec arrêt", "1NT, 8-10 HL with a stopper")
		}
	}
	if tr.check(hl >= 11, "11 HL et plus → décision sur la force combinée", "11+ HL → decision on combined strength", pts(hl, "HL")) {
		tr.in()
		return e.concludeFrom(p, tr)
	}
	tr.note("aucune réponse ne s'applique → Passe", "no response applies → Pass")
...
func (e *Engine) answerSpoutnik(p *playerState, majors [4]bool) (Call, meaning) {
	...
	tr := e.tr
	tr.note("le partenaire a fait un contre Spoutnik : 4 cartes dans la majeure libre", "partner made a negative double: four cards in the free major")
	if tr.check(best >= 0, "4 cartes dans la majeure promise → le fit (saut dès 17 HLD, manche dès 19)",
		"four cards in the promised major → the fit (jump from 17 HLD, game from 19)", cards(h, Spades)+", "+cards(h, Hearts)) {
		hld := h.HLD(best)
		c := e.cheapestCall(best.Strain())
	...
	if opp >= 0 && tr.check(h.Stopper(opp), "pas le fit, arrêt dans leur couleur → Sans-Atout (3SA dès 18 H)",
		"no fit, a stopper in their suit → notrump (3NT from 18 H)", cards(h, opp)+", "+pts(h.H(), "H")) {
		c := e.cheapestCall(SNoTrump)
		hp := h.H()
		switch {
	...
	if e.openCall.Strain <= SSpades {
		...
		if tr.check(h.Len(os) >= 6, "6 cartes dans la couleur d'ouverture → la répéter", "six cards in the opened suit → rebid it", cards(h, os)) {
	...
	for _, s := range []Suit{Clubs, Diamonds, Hearts, Spades} {
		...
		if c.Level <= 2 && e.legal(p.seat, c) {
			tr.check(true, "une autre couleur quatrième → la nommer", "another four-card suit → bid it", cards(h, s))
...
func (e *Engine) respondRubensohl(p *playerState, v Suit) (Call, meaning) {
	...
	hl := h.HL()
	tr := e.tr
	tr.note("intervention sur l'ouverture de 1SA ("+suitSymbol[v]+") : Rubensohl", "overcall of the 1NT opening ("+suitSymbol[v]+"): Rubensohl")
	...
	// below.
	if tr.check(hl >= 6 && h.Len(v) >= 2 && h.sortedLens()[0] < 5 && e.legal(p.seat, doubleCall),
		"6 HL, 2 cartes dans leur couleur, pas de cinquième → contre (tendance Stayman)",
		"6 HL, two cards in their suit, no five-card suit → double (Stayman tendency)", pts(hl, "HL")+", "+shape(h)) {
	...
	if hl <= 7 {
		for _, s := range []Suit{Diamonds, Hearts, Spades} {
			...
			c := bid(2, s.Strain())
			if e.legal(p.seat, c) {
				tr.check(true, "0-7 HL et une cinquième au-dessus de leur couleur → naturel au palier de 2",
					"0-7 HL and a five-card suit above theirs → natural at the two level", cards(h, s))
		...
		for i, s := range ladder {
			...
			if s == v {
				if h.Len(v) <= 1 && e.legal(p.seat, c) {
					tr.check(true, "8 HL, singleton ou chicane dans leur couleur → Texas impossible (demande des majeures)",
						"8 HL, singleton or void in their suit → impossible Texas (asks for the majors)", cards(h, v))
			...
			if h.Len(s) >= 5 && e.legal(p.seat, c) {
				cFR, cEN := callSym(c)
				tr.check(true, "8 HL et une cinquième → Texas Rubensohl ("+cFR+" pour "+suitSymbol[s]+")",
					"8 HL and a five-card suit → Rubensohl transfer ("+cEN+" for "+suitSymbol[s]+")", cards(h, s))
	...
	if tr.check(hl >= 10, "10 HL et plus : la manche", "10+ HL: game", pts(hl, "HL")) {
		l := h.sortedLens()
		flat433 := l[0] == 4 && l[1] == 3 && l[2] == 3 && l[3] == 3 && !h.Longest().IsMajor()
		if tr.check(h.Stopper(v) || flat433, "arrêt dans leur couleur (ou 4-3-3-3 mineure) → 3SA",
			"a stopper in their suit (or 4-3-3-3 with a minor) → 3NT", cards(h, v)) {
			c := bid(3, SNoTrump)
			if e.legal(p.seat, c) {
		...
		c := bidSuit(3, Spades)
		if e.legal(p.seat, c) {
			tr.note("sinon → 3♠, demande d'arrêt", "otherwise → 3♠, stopper ask")
	...
	tr.note("trop faible pour agir → Passe", "too weak to act → Pass")
...
func (e *Engine) afterRubensohlMinorTexas(p *playerState, s Suit) (Call, meaning) {
	partner := e.ps[partnerOf(p.seat)]
	tr := e.tr
	tr.note("le partenaire a rectifié mon Texas mineur (Rubensohl)", "partner completed my minor transfer (Rubensohl)")
	if tr.check(p.hand.HLD(s)+partner.shownMin >= 25, "25 réunis avec son minimum → la manche",
		"25 together with partner's minimum → game", fmt.Sprintf("%d + %d", p.hand.HLD(s), partner.shownMin)) {
		if c, mn, ok := e.concludeTry(p); ok {
			return c, mn
		}
	}
	tr.note("→ Passe, arrêt au palier de 3", "→ pass, stop at the three level")
...
func (e *Engine) rubensohlDoubleAnswer(p *playerState) (Call, meaning) {
	...
	tr := e.tr
	tr.note("le partenaire a contré (Rubensohl, tendance Stayman) : une majeure quatrième ?", "partner doubled (Rubensohl, Stayman tendency): a four-card major?")
	for _, s := range []Suit{Hearts, Spades} {
		if s == v || h.Len(s) < 4 {
			continue
		}
		c := e.cheapestCall(s.Strain())
		if e.legal(p.seat, c) {
			tr.check(true, "majeure quatrième → la nommer", "a four-card major → bid it", cards(h, s))
			return c, m(-1, -1, "répond sa majeure au Contre Rubensohl", "answers the Rubensohl double with a major").withLen(s, 4)
		}
	}
	if tr.check(h.Stopper(v) && h.H() >= 17, "arrêt dans leur couleur et 17 H → passer, le contre devient punitif",
		"a stopper in their suit and 17 H → pass, the double becomes penalty", cards(h, v)+", "+pts(h.H(), "H")) {
	...
	tr.note("pas de majeure quatrième → Passe", "no four-card major → Pass")
...
func (e *Engine) rubensohlAskAnswer(p *playerState) (Call, meaning) {
	...
	tr := e.tr
	tr.note("le partenaire demande les majeures (Texas impossible) : majeure quatrième, sinon arrêt",
		"partner asks for the majors (impossible transfer): a four-card major, else a stopper")
	for _, s := range []Suit{Hearts, Spades} {
		if s == v || h.Len(s) < 4 {
			continue
		}
	...
	if tr.check(h.Stopper(v), "arrêt dans leur couleur → 3SA", "a stopper in their suit → 3NT", cards(h, v)) {
		c := bid(3, SNoTrump)
		if e.legal(p.seat, c) {
	...
	if e.legal(p.seat, c) {
		tr.note("ni majeure ni arrêt → cue-bid", "neither major nor stopper → cue-bid")
...
func (e *Engine) rubensohlStopperAskAnswer(p *playerState) (Call, meaning) {
	...
	tr := e.tr
	tr.note("le partenaire demande un arrêt pour 3SA (3♠)", "partner asks for a stopper for 3NT (3♠)")
	if tr.check(h.Stopper(v), "arrêt dans leur couleur → 3SA", "a stopper in their suit → 3NT", cards(h, v)) {
		c := bid(3, SNoTrump)
		if e.legal(p.seat, c) {
	...
	c := e.cheapestCall(v.Strain())
	if e.legal(p.seat, c) {
		tr.note("pas d'arrêt → cue-bid", "no stopper → cue-bid")
...
func (e *Engine) lightOpenerRebid(p *playerState) (Call, meaning, bool) {
	...
	tr := e.tr
	tr.note("ouverture légère (sous 12 H, troisième ou quatrième position) : elle a déjà tout dit",
		"light opening (under 12 H, third or fourth seat): it has already said everything")
	if !tr.check(forced, "le partenaire force à reparler (Drury, rencontre)", "partner forces a rebid (Drury, fit-showing jump)", "") {
		tr.note("→ Passe", "→ Pass")
	...
	repFR, repEN := callSym(rep)
	tr.note("répéter la couleur d'ouverture au plus bas, sans rien promettre de plus → "+repFR,
		"rebid the opened suit as cheaply as possible, promising nothing more → "+repEN)
...
func (e *Engine) openerRebid(p *playerState) (Call, meaning) {
	...
	if rm != nil && rm.spoutnik && hasResp && rsc.Call.Kind == KindDouble {
		return e.answerSpoutnik(p, rm.spoutnikSuits)
	}
	...
	case oc == bid(1, SNoTrump) || oc == bid(2, SNoTrump):
		...
		if rm != nil && rm.hasTexas {
			...
			return e.minorTransferAnswer(p, rm.texas)
		}
		if rm != nil && rm.rubensohlDouble {
			return e.rubensohlDoubleAnswer(p)
		}
		if rm != nil && rm.rubensohlAsk {
			return e.rubensohlAskAnswer(p)
		}
		if rm != nil && rm.rubensohlStopperAsk {
			return e.rubensohlStopperAskAnswer(p)
		}
		return e.concludeHandoff(p)
	case oc == bid(2, SClubs):
	...
	case oc == bid(3, SNoTrump):
		if hasResp && rsc.Call.IsBid() && rsc.Call.Strain <= SDiamonds {
			return e.affranchieRebid(p)
		}
		return e.concludeHandoff(p)
	case oc.Level == 2 && (oc.Strain == SHearts || oc.Strain == SSpades):
		if hasResp && rsc.Call == bid(2, SNoTrump) {
			return e.weak2Feature(p, Suit(oc.Strain))
		}
		return e.concludeHandoff(p)
	case oc.Level == 1 && oc.Strain <= SSpades:
...
func (e *Engine) gfRebid(p *playerState) (Call, meaning) {
	...
	long := h.Longest()
	tr := e.tr
	tr.note("2♦ fort (24 HL et plus) après la réponse par paliers d'As : décrire la main", "strong 2♦ (24+ HL) after the ace-step answer: describe the hand")
	if tr.check(h.Len(long) >= 5, "couleur cinquième → la nommer, forcing de manche", "five-card suit → bid it, game forcing", cards(h, long)) {
	...
	// aux Rois").
	if isGame(c) {
		if kc, kmn, ok := e.strongTwoDKingAsk(p); tr.check(ok, "les 4 As localisés → 4SA, appel aux Rois",
			"all four aces located → 4NT king ask", "") {
			return kc, kmn
		}
	}
	...
	partner := e.ps[partnerOf(p.seat)]
	if cMin := h.H() + partner.shownMin; tr.check(cMin >= 33, "33 H réunis → chelem à Sans-Atout (grand dès 37)",
		"33 H between the hands → notrump slam (grand from 37)", fmt.Sprintf("%d + %d = %d", h.H(), partner.shownMin, cMin)) {
		lvl := 6
		if cMin >= 37 {
			lvl = 7
		}
	...
	cFR, cEN := callSym(c)
	tr.note("jeu régulier → redemande par défaut : "+cFR, "balanced hand → default rebid: "+cEN)
...
func (e *Engine) weak2Feature(p *playerState, M Suit) (Call, meaning) {
	...
	hld := h.HLD(M)
	tr := e.tr
	tr.note("le partenaire demande par 2SA (relais) : minimum ou maximum, et où est la force",
		"partner asks with the 2NT relay: minimum or maximum, and where the strength is")
	if tr.check(hld <= 11, "minimum, 9-11 HLD → répéter la couleur à 3", "minimum, 9-11 HLD → repeat the suit at three", pts(hld, "HLD")) {
	...
	for s := Spades; s >= Clubs; s-- {
		if s != M && h.Len(s) >= 3 && (h.HasCard(s, 'A') || h.HasCard(s, 'K')) {
			c := e.cheapestCall(s.Strain())
			if c.Level <= 3 {
				tr.check(true, "maximum : As ou Roi troisième à "+suitSymbol[s]+" → le montrer",
					"maximum: ace or king third in "+suitSymbol[s]+" → show it", cards(h, s))
	...
	for s := Clubs; s <= Spades; s++ {
		if s != M && h.Len(s) == 1 {
			tr.check(true, "maximum sans force extérieure, singleton à "+suitSymbol[s]+" → le montrer au palier de 4",
				"maximum without outside strength, a singleton in "+suitSymbol[s]+" → show it at the four level", cards(h, s))
	...
	tr.note("maximum, ni force extérieure ni singleton → 3SA", "maximum, neither outside strength nor singleton → 3NT")
...
func (e *Engine) naturalRebid(p *playerState, rsc SeatCall, hasResp bool) (Call, meaning) {
	...
	if rm != nil && (rm.drury || rm.druryShort) {
		...
		if rm.drury {
			return e.druryRebid(p, os)
		}
		return e.druryShortRebid(p, os)
	}
		...
		case 2:
			...
			case os.IsMajor() && tr.check(hld >= 17, "17-21 HLD → essai de manche",
				...
				if c := bidSuit(3, os); e.legal(p.seat, c) {
					tr.note("2SA déjà dépassé → essai en répétant l'atout au palier de 3", "2NT already passed → try by repeating the trump at the three level")
				...
				if c := e.cheapestCall(os.Strain()); e.legal(p.seat, c) {
					tr.note("plus de place pour un essai → la manche", "no room left for a try → game")
...
func (e *Engine) openerReopen(p *playerState, os Suit) (Call, meaning) {
	...
	last, lastSeat, hasBid := e.lastBid()
	tr := e.tr
	tr.note("le partenaire a passé : réouvrir ou laisser jouer", "partner passed: reopen or let them play")
	if !tr.check(hasBid && sideOf(lastSeat) != sideOf(p.seat), "les adversaires ont la dernière enchère",
		"the opponents hold the last bid", "") {
		tr.note("notre ouverture tient → Passe", "our opening stands → Pass")
		return passCall, noInfo()
	}
	if tr.check(hp >= 18 && last.Level <= 3 && e.legal(p.seat, doubleCall),
		"18 H et plus, enchère adverse au plus au palier de 3 → contre de réveil",
		"18+ H, their bid at the three level at most → reopening double", pts(hp, "H")) {
		return doubleCall, m(18, 23, "contre de réveil, jeu fort", "strong reopening double").asForcing()
	}
	if tr.check(h.Len(os) >= 6, "6 cartes dans la couleur d'ouverture → la répéter (au plus au palier de 3)",
		"six cards in the opened suit → rebid it (at most at the three level)", cards(h, os)) {
		c := e.cheapestCall(os.Strain())
		if c.Level <= 3 {
...
func (e *Engine) reopenBid(p *playerState, opp Suit) (Call, meaning) {
	...
	balanced := h.IsRegular() || h.IsSemiRegular()
	tr := e.tr
	tr.note("réveil : leur ouverture au palier de 1 s'éteindrait après deux passes, les zones sont décalées",
		"balancing seat: their one-level opening would die after two passes, the ranges shift")

	// Two-suited reopenings first, as in the direct seat [V-1].
	if c, mn, ok := e.reopenTwoSuiter(p, opp); tr.check(ok, "bicolore → réveil bicolore [V-1]",
		"two-suiter → two-suited reopening [V-1]", shape(h)) {
		return c, mn
	}
	// 2SA is natural in the balancing seat: 17-19 HL with a stopper [V-2].
	if tr.check(hl >= 17 && hl <= 19 && balanced && h.Stopper(opp),
		"17-19 HL, régulière, arrêt dans leur couleur → 2SA [V-2]",
	...
	if tr.check(hl >= 14 && e.legal(p.seat, doubleCall), "14 HL et plus : la valeur d'une ouverture → contre de réveil [V-3]",
		"14+ HL: opening values → reopening double [V-3]", pts(hl, "HL")) {
		tr.in()
		if c, mn, ok := e.reopenLongSuit(p, opp); tr.check(ok, "sauf avec une couleur longue et forte → la nommer",
			"unless holding a long strong suit → bid it", callOrEmpty(c, ok)) {
			return c, mn
		}
		if tr.check(hp >= 14 && hp <= 16 && balanced && h.reopenHold(opp),
			"14-16 H, régulière, tenue dans leur couleur → contre puis Sans-Atout",
			"14-16 H, balanced, a hold in their suit → double then notrump", pts(hp, "H")) {
		...
		tr.note("→ contre de réveil", "→ reopening double")
	...
	if c, mn, ok := e.reopenJump(p, opp); tr.check(ok, "six cartes → réveil à saut [V-5]",
		"six cards → jump reopening [V-5]", callOrEmpty(c, ok)) {
		return c, mn
	}
	...
	if c, mn, ok := e.reopenSuit(p, opp, 1); tr.check(ok, "belle cinquième au palier de 1, 8-13 HL → la nommer [V-6]",
	...
	if tr.check(hl >= 9 && hl <= 13 && balanced && h.reopenHold(opp),
		...
		"9-13 HL, balanced, a stopper or three small cards in their suit → 1NT [V-4]", pts(hl, "HL")+", "+cards(h, opp)) {
	...
	if c, mn, ok := e.reopenSuit(p, opp, 2); tr.check(ok, "belle cinquième au palier de 2 → la nommer [V-6]",
		"good five-card suit at the two level → bid it [V-6]", callOrEmpty(c, ok)) {
		return c, mn
	}
	// Reopening double on the takeout shape: short in the opened suit, three
	// cards or more everywhere else, and 8 H is enough [V-7].
	if tr.check(hp >= 8 && e.legal(p.seat, doubleCall) && takeoutReopenShape(h, opp),
		"8 H, court dans leur couleur, 3 cartes partout ailleurs → contre d'appel [V-7]",
		"8 H, short in their suit, three cards everywhere else → takeout double [V-7]", pts(hp, "H")+", "+shape(h)) {
	...
	if hp >= 9 {
		for _, s := range []Suit{Spades, Hearts, Diamonds} {
			...
			if c := e.cheapestCall(s.Strain()); c.Level == 1 && !reopenReserved(c, opp) {
				tr.check(true, "9 H et une belle quatrième au palier de 1 → la nommer [V-8]",
					"9 H and a strong four-card suit at the one level → bid it [V-8]", cards(h, s))
	...
	tr.note("aucun réveil ne s'applique → Passe", "no balancing call applies → Pass")
...
func (e *Engine) reopenDoubleNTRebid(p *playerState, opp Suit) (Call, meaning) {
	partner := e.ps[partnerOf(p.seat)]
	c := e.cheapestCall(SNoTrump)
	e.tr.note("contre de réveil avec 14-16 H régulier : Sans-Atout prévu", "reopening double with a balanced 14-16: notrump planned")
	if e.tr.check(partner.lastM != nil && partner.lastM.maxPts >= 0 && partner.lastM.maxPts <= 10 &&
		c.Level <= 2 && e.legal(p.seat, c), "le partenaire est limité (10 H au plus) → Sans-Atout",
		"partner is limited (10 H at most) → notrump", "") {
...
func (e *Engine) advanceMichaels(p *playerState, pm *meaning) (Call, meaning) {
	...
	tr := e.tr
	tr.note("le partenaire a fait un Michaels ("+suitSymbol[s1]+" et "+suitSymbol[s2]+", 5-5) : choisir la couleur, au palier des atouts",
		"partner made a Michaels cue-bid ("+suitSymbol[s1]+" and "+suitSymbol[s2]+", 5-5): pick the suit, at the level of the trumps")
	suitsVal := cards(h, s1) + ", " + cards(h, s2)

	if tr.check(lBest <= 2 && lWorst <= 2 && n3Bid, "l'adversaire a parlé et 2 cartes au plus dans ses couleurs → Passe",
		"the opponent bid and two cards at most in partner's suits → Pass", suitsVal) {
	...
	if tr.check(n3Bid && lBest == 3 && hl < 8, "l'adversaire a parlé, 3 cartes et moins de 8 HL → Passe",
		"the opponent bid, three cards and under 8 HL → Pass", suitsVal+", "+pts(hl, "HL")) {
	...
	cFR, cEN := callSym(c)
	tr.check(true, "la plus longue de ses couleurs, au palier : 4 avec 13 HL ou 10 atouts, 3 avec 10 HL ou 9 atouts → "+cFR,
		"the longer of partner's suits, at the level: 4 with 13 HL or 10 trumps, 3 with 10 HL or 9 trumps → "+cEN,
		fmt.Sprintf("%d %s + 5, %s", lBest, suitSymbol[best], pts(hl, "HL")))
...
func (e *Engine) advanceLandy(p *playerState) (Call, meaning) {
	...
	lh, ls := h.Len(Hearts), h.Len(Spades)
	tr := e.tr
	tr.note("le partenaire a fait un Landy (les deux majeures) : choisir la majeure, au palier des atouts",
		"partner bid Landy (both majors): pick the major, at the level of the trumps")
	...
	if tr.check(lh <= 2 && ls <= 2 && h.Len(Clubs) >= 5, "pas de majeure et 5 Trèfles → passer 2♣",
		"no major and five clubs → pass 2♣", cards(h, Hearts)+", "+cards(h, Spades)+", "+cards(h, Clubs)) {
	...
	if tr.check(c.Level > level, "le palier dépasse ce que le fit et la zone paient → Passe",
		"the level is past what the fit and the zone pay for → Pass", "") {
		return passCall, m(-1, -1,
	...
	cFR, cEN := callSym(c)
	tr.note("la plus longue majeure, au palier : 4 avec 13 HLD ou 10 atouts, 3 avec 10 HLD ou 9 atouts → "+cFR,
		"the longer major, at the level: 4 with 13 HLD or 10 trumps, 3 with 10 HLD or 9 trumps → "+cEN)
...
func (e *Engine) afterLandy(p *playerState) (Call, meaning) {
	...
	if psc, ok := e.lastCallBy(partner.seat); ok && psc.Call == bidSuit(2, Hearts) &&
		partner.shownLens[Hearts] <= 3 && e.tr.check(h.Len(Spades) >= 5 && h.Len(Hearts) == 4,
		"le partenaire a choisi 2♥ sans soutien promis ; 5 ♠ et 4 ♥ seulement → rectifier à 2♠",
		"partner chose 2♥ without promised support; five ♠ and only four ♥ → correct to 2♠", cards(h, Spades)+", "+cards(h, Hearts)) {
		c := bidSuit(2, Spades)
		if e.legal(p.seat, c) {
			return c, m(-1, -1,
...
func (e *Engine) advance(p *playerState) (Call, meaning) {
	...
	if ctx := e.concludeContext(p); ctx.hasBid && !ctx.ours && ctx.hasFit &&
		...
		ctx.tr = tr
		mark := tr.mark()
		tr.note("les adversaires ont demandé la manche et nous avons un fit : surenchérir, sacrifier ou les laisser jouer",
			"the opponents bid game and we hold a fit: outbid them, sacrifice or let them play")
	...
	if !psc.Call.IsBid() && psc.Call.Kind != KindDouble {
		tr.check(false, "le partenaire vient d'enchérir (couleur, Sans-Atout ou contre)",
			"partner has just bid (a suit, notrump or a double)", "")
	}
...
func (e *Engine) advanceReopenSuit(p *playerState, s Suit) (Call, meaning) {
	...
	tr := e.tr
	tr.note("le partenaire a réveillé à la couleur (8-13 HL, sans l'ouverture) : répondre sur ses zones",
		"partner reopened in a suit (8-13 HL, no opening values): answer on its ranges")
	...
	if sup >= 4 {
		for _, cand := range []Suit{Clubs, Diamonds, Hearts, Spades} {
			...
			if c, mn, ok := e.tryRencontre(p, s, cand, 4, 1); ok {
				tr.check(true, "4 atouts et belle cinquième à "+suitSymbol[cand]+" → rencontre",
					"four trumps and a good five cards in "+suitSymbol[cand]+" → meeting bid", cards(h, cand))
				return c, mn
			}
		}
	}
	if tr.check(sup >= 3, "3 atouts et plus", "three or more trumps", cards(h, s)) {
		...
		case tr.check(hld >= 11, "11-12 HLD → soutien à saut", "11-12 HLD → jump raise", pts(hld, "HLD")):
			c := e.cheapestCall(s.Strain())
	...
	if hasOpp && tr.check(h.reopenHold(opp), "arrêt ou trois petites cartes dans leur couleur → Sans-Atout (1SA 9-12 H, 2SA 13-15 H)",
		"a stopper or three small cards in their suit → notrump (1NT 9-12 H, 2NT 13-15 H)", cards(h, opp)+", "+pts(hp, "H")) {
	...
	if tr.check(hl >= 12, "12 HL et plus sans enchère évidente → cue-bid", "12+ HL with no obvious bid → cue-bid", pts(hl, "HL")) {
	...
	if own := h.Longest(); tr.check(own != s && h.Len(own) >= 5 && h.GoodSuit(own) && hl >= 10 && !(hasOpp && own == opp),
		"belle cinquième à soi et 10 HL → la nommer, non forcing (misfit)", "a good five-card suit of one's own and 10 HL → bid it, not forcing (misfit)",
		cards(h, own)+", "+pts(hl, "HL")) {
...
func (e *Engine) answerReopenDouble(p *playerState, oppSuit Suit, forced bool) (Call, meaning) {
	...
	tr := e.tr
	tr.note("le partenaire a contré en réveil (dès 8 H) : répondre comme face à une main faible",
		"partner made a balancing double (from 8 H): answer as though partner were weak")
	...
	// bid describes it.
	if tr.check(hp >= 13, "13 H et plus : au-delà de l'échelle", "13+ H: beyond the ladder", pts(hp, "H")) {
		if tr.check(hold && !hasMajor4 && (h.IsRegular() || h.IsSemiRegular()) && hp <= 14,
			"13-14 H, régulière, tenue dans leur couleur, sans majeure quatrième → 2SA",
			"13-14 H, balanced, a hold in their suit, no four-card major → 2NT", shape(h)) {
		...
		if c.Level <= 3 && e.legal(p.seat, c) {
			tr.note("sinon → cue-bid, forcing", "otherwise → cue-bid, forcing")
	...
	jumps, lo, hi := 0, 0, 10
	switch {
	case tr.check(bestLen >= 5 && hp >= 11, "5 cartes et 11-12 H → double saut", "five cards and 11-12 H → double jump", cards(h, best)+", "+pts(hp, "H")):
		jumps, lo, hi = 2, 11, 12
	case tr.check(bestLen >= 5 && hp >= 8, "5 cartes et 8-10 H → saut", "five cards and 8-10 H → jump", cards(h, best)+", "+pts(hp, "H")):
		jumps, lo, hi = 1, 8, 10
	case tr.check(bestLen >= 4 && hp >= 11, "4 cartes et 11-12 H → saut", "four cards and 11-12 H → jump", cards(h, best)+", "+pts(hp, "H")):
		jumps, lo, hi = 1, 11, 12
	}
	...
	if jumps == 0 && tr.check(hp >= 9 && hp <= 12 && hold && !hasMajor4,
		"9-12 H, tenue dans leur couleur, sans majeure quatrième → 1SA", "9-12 H, a hold in their suit, no four-card major → 1NT", pts(hp, "H")) {
	...
	if jumps == 0 && tr.check(!forced && hp < 8, "libéré par l'enchère adverse et moins de 8 H → Passe",
		"freed by the opponents' bid and under 8 H → Pass", pts(hp, "H")) {
	...
	cFR, cEN := callSym(c)
	tr.note("→ "+cFR+" (la couleur la plus longue, majeure quatrième d'abord)", "→ "+cEN+" (the longest suit, a four-card major first)")
