...
func (e *Engine) respondMajor(p *playerState, M Suit) (Call, meaning) {
	...
	if M == Hearts && h.Len(Spades) >= 4 && hl >= 6 && sup < 3 {
...
func (e *Engine) respondMinor(p *playerState, ms Suit) (Call, meaning) {
	...
	if c, mn, ok := e.rencontreCall(p, ms, other, 4, 1); ok {
		return c, mn
	}
	if ms == Clubs && h.Len(Diamonds) >= 4 {
...
func (e *Engine) respondNT(p *playerState, base, oMin int) (Call, meaning) {
	...
	if (lh >= 5 && ls < 4) || (ls >= 5 && lh < 4) {
		...
		if base == 1 && hp >= 7 && hp <= 8 && hasMisereDoreeSingleton(h, M) {
			c := bid(2, SClubs)
	...
	if base == 2 {
		if c, mn, ok := e.strongMinorTwoSuiter(p); ok {
			return c, mn
		}
	}
...
func (e *Engine) transferAnswer(p *playerState, t Suit) (Call, meaning) {
	...
	if t.IsMajor() && h.Len(t) >= 4 && p.shownMin+2 <= p.hand.H() && c.Level <= 2 {
...
func (e *Engine) strongRebid(p *playerState) (Call, meaning) {
	...
	if (h.IsRegular() || h.IsSemiRegular()) && hp >= 22 {
...
func (e *Engine) naturalRebid(p *playerState, rsc SeatCall, hasResp bool) (Call, meaning) {
	...
	if os.IsMajor() && rc == bid(2, SNoTrump) && rm != nil && rm.lens[os] >= 3 {
	...
	if rc.Strain == os.Strain() {
		...
		case 2:
			...
			case hld >= 17 && os.IsMajor():
				...
				if c, mn, ok := e.helpSuitGameTry(p, os); ok {
					p.invited = true
					return c, mn
				}
		...
		case 3:
			...
			if hld+resp.shownMin >= 27 && os.IsMajor() {
	...
	// Notrump responses.
	if rc.Strain == SNoTrump && rc.Level == 1 {
...
func (e *Engine) rebidOverOneNT(p *playerState, os Suit) (Call, meaning) {
	...
	switch {
	case os.IsMajor() && h.Len(os) >= 7 && h.HLD(os)+e.ps[partnerOf(p.seat)].shownMin >= gameThreshold(os, true):
...
func (e *Engine) rebidOverNewSuit(p *playerState, os, rs Suit, respLevel int) (Call, meaning) {
	...
	if balanced && hp <= 14 && respLevel == 2 && e.ntSafe(p) {
	...
	// game.
	if sup == 3 && rs.IsMajor() && respLevel == 2 && !e.uncontested(p.seat) {
	...
	if sup == 3 && rs.IsMajor() && respLevel == 1 && !e.uncontested(p.seat) {
		hld := e.hldAgainstTheirBidding(p, rs)
