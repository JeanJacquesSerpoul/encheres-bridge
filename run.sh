#!/usr/bin/env bash
# run.sh — sert cli/ sur http://localhost:<port>/ et ouvre l'application dans
# le navigateur.
#
...
# calcul du PAR. Il est compilé dans ./bids (ignoré par git) et conservé : le
# lancement suivant repart de là. Ctrl+C l'arrête.
#
...
# La page elle-même : c'est le signal qu'attendre avant de l'ouvrir, plutôt
# qu'un délai au jugé.
if command -v curl >/dev/null 2>&1; then
    probe() { curl -sf -m 2 "http://localhost:$port/index.html" >/dev/null 2>&1; }
elif command -v wget >/dev/null 2>&1; then
    probe() { wget -q -T 2 -O /dev/null "http://localhost:$port/index.html" >/dev/null 2>&1; }
else
...
# Le moteur est versionné ; -f le recompile depuis les sources Go locales,
# après une modification du moteur.
if [ "$force_wasm" -eq 1 ] || [ ! -f "$root/cli/bids.wasm" ]; then
    "$root/build-wasm.sh"
fi

echo "Compilation de $(basename "$bin")..."
(cd "$root" && CGO_ENABLED=0 go build -trimpath -o "$bin" ./serve)

# Le serveur garde le premier plan : Ctrl+C lui parvient directement. L'ouverture
# de la page, elle, attend en arrière-plan que le serveur réponde.
