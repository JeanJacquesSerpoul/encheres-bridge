...
- La barre au-dessus de la table : **Donne aléatoire**, dont la flèche ouvre **Donne exemple** et **Charger un fichier .pbn** (et la photo des quatre mains quand le serveur IA est activé), puis le menu **Partager** : **Copier le lien**, **Sauver le PBN** et **Texte de la donne (format PBN)**. Un fichier de tournoi (plusieurs `[Board]`) fait apparaître un sélecteur **Donne à utiliser**.
...
- **À la première visite**, la table porte la **donne exemple**. Ensuite, la **dernière donne complète** (quatre mains de 13 cartes) est retenue dans le navigateur et revient à chaque ouverture de la page — jamais la donne exemple, qui n'est qu'à un clic.
